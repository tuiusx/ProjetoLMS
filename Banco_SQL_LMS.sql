...
(
	...
	STATUS_ATIVIDADE_VINCULADA VARCHAR (200) NOT NULL,
	DATA_INIC_RESPOSTAS DATETIME NOT NULL,
	DATA_FIM_RESPOSTAS	DATETIME NOT NULL,

	CONSTRAINT PK_ATIVIDADE_VINCULADA PRIMARY KEY (ID),

	CONSTRAINT FK_ATIVIDADE_VINCULADA_ID_ATIVIDADE FOREIGN KEY (ID_ATIVIDADE)
		REFERENCES ATIVIDADE (ID) ,

	CONSTRAINT FK_ATIVIDADE_VINCULADA_ID_PROFESSOR FOREIGN KEY (ID_PROFESSOR)
		REFERENCES PROFESSOR (ID),

	CONSTRAINT FK_ATIVIDADE_VINCULADA_ID_DISCIPLINA_OFERTADA FOREIGN KEY (ID_DISCIPLINA_OFERTADA)
		REFERENCES DISCIPLINA_OFERTADA (ID),

	CONSTRAINT CK_STATUS_ATIVIDADE_VINCULADA CHECK (STATUS_ATIVIDADE_VINCULADA IN ('DISPONIBILIZADA', 'ABERTA', 'FECHADA','ENCERRADA', 'PRORROGADA')),
);
...
(
	...
	DATA_ENTREGA DATETIME NOT NULL CONSTRAINT DF_DATA_ENTREGA DEFAULT (GETDATE()),
	STATUS_ENTREGA VARCHAR(50) NOT NULL CONSTRAINT DF_STATUS_ENTREGA DEFAULT ('ENTREGUE'),
	ID_PROFESSOR INT,
	NOTA NUMERIC(4,2),
	DATA_AVALIACAO DATE,
	OBS VARCHAR(300),
	...
	CONSTRAINT FK_ENTREGA_ID_ALUNO FOREIGN KEY (ID_ALUNO)
		REFERENCES ALUNO (ID),

	CONSTRAINT FK_ENTREGA_ID_ATIVIDADE_VINCULADA FOREIGN KEY (ID_ATIVIDADE_VINCULADA)
		REFERENCES ATIVIDADE_VINCULADA (ID),

	CONSTRAINT FK_ENTREGA_ID_PROFESSOR FOREIGN KEY (ID_PROFESSOR)
		REFERENCES PROFESSOR (ID),

	
	CONSTRAINT CK_STATUS_ENTREGA CHECK (STATUS_ENTREGA IN ('ENTREGUE', 'CORRIGIDO')),
...
(
	...
	STATUS_MENSAGEM VARCHAR(50) NOT NULL CONSTRAINT DF_STATUS_MENSAGEM DEFAULT ('RESPONDIDO'),
	DT_ENVIO DATETIME NOT NULL CONSTRAINT DF_DT_ENVIO DEFAULT (GETDATE()) ,
	DT_RESPOSTA DATETIME,
	RESPOSTA VARCHAR (8000),

	CONSTRAINT PK_MENSAGEM PRIMARY KEY (ID),
	CONSTRAINT FK_MENSAGEM_ID_ALUNO FOREIGN KEY (ID_ALUNO)
			REFERENCES ALUNO (ID),
	CONSTRAINT FK_MENSAGEM_ID_PROFESSOR FOREIGN KEY (ID_PROFESSOR)
			REFERENCES PROFESSOR (ID),

	CONSTRAINT CK_MENSAGEM_STATUS CHECK ( STATUS_MENSAGEM IN ('ENVIADO','LIDO','RESPONDIDO')),
